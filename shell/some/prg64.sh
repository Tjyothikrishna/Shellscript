#!/bin/bash
read -p "Provide VG name to create 20 Lvs in it and mount the same" vgname
for i in `seq 1 20`
do
  lvcreate -L 64M -n imagelv$i $vgname >/dev/null 2>&1
  if [ $? -ne 0 ]
  then 
  ...
  mkfs.ext4 -F /dev/$vgname/imagelv$i >/dev/null 2>&1
  if [ $? -ne 0 ]
  then 
      echo "Could not create filesystem on /imagefs$i"
  fi
  echo "/dev/$vgname/imagelv$i	/imagefs$i	ext4	defaults 	0 0">>/etc/fstab 
  if [ $? -ne 0 ]
  then 
